...
public class ImageService {
	 ...
	 * 
	 * 이 때는 dao, mapper SELECT KEY를 사용해서 값을 생성하면 
	 * insert 후에 그 값을 사용할 수 있다.
	 */
	public int insert(ImageVO imageVO) {
		
		List<String> fileList = imageVO.getImg_files();

		// 여러개의 파일 중 0번 파일을 대표 파일로 업로드
		...
		// ImageFilesVO에 img_files_p_code 칼럼에 
	...
	public ImageVO findBySeq(String data_seq) {
		...
		ImageVO imgVO = imDao.findBySeq(data_seq);
		return imgVO;
	}
