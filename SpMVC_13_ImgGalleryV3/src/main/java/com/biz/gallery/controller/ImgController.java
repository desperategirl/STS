...
public class ImgController {
	...
	public String list(Model model) {
	  ...
	  @RequestMapping(value = "/upload", method = RequestMethod.GET) public String
	  upload(@ModelAttribute("imageVO") ImageVO imageVO, Model model, HttpSession
	  httpSession) {
	  
		/*
		 * MemberVO member = (MemberVO) httpSession.getAttribute("MEMBER"); if (member
		 * == null) {
		 * 
	  ...
	  model.addAttribute("BODY", "UPLOAD"); model.addAttribute("imageVO", imageVO);
	  return "home"; }
	  
	  @RequestMapping(value = "/upload", method = RequestMethod.POST) public String
	  upload(@ModelAttribute("imageVO") ImageVO imageVO, SessionStatus status) {
	  
	  log.debug("업로드:" + imageVO.toString());
	  
	  imService.insert(imageVO); status.setComplete(); return
	  "redirect:/image/list";
	...
	/*
	 * @RequestParam : ?변수=값 형식 으로 전송하고 변수에서 수신
	 * 
	...
	/*
	 * @RequestMapping(value="/update/{img_seq}", method=RequestMethod.POST) public
	 * String update(
	 * 
	 * @ModelAttribute("imageVO") ImageVO imageVO, SessionStatus status) {
	 * 
	 * log.debug("Update Post : " + imageVO.getImg_files().toString());
	 * 
	 * // 이미지 이름이 기존의 이미지와 다르면 기존 이미지를 삭제 // 서비스에 해당 기능 구현 int ret =
	 * imService.update(imageVO);
	 * 
	 * 
	 * sesssionAttributes를 사용할때 객체가 서버 메모리에 남아서 계속 유지되는 상태이므로 insert, update 등을 수행할때
	 * 그 정보를 계속 사용해서 form에 값이 나타나게 된다.
	 * 
	 * 그것을 방지하기 위해서 insert, update가 완료된 후에는 반드시 SessionStatus의 setComplete() method를
	 * 호출해서 sessionAttribute를 해제 해주어야 한다.
	 * 
	 * 
	 * status.setComplete(); return "redirect:/image/list"; }
	 */
	...
	/*
	 * MultipartHttpServletRequest 다중파일 수신하여 업로드를 수행한후 파일리스트를 view와 결합하여 DB에 저장전
	 * 보여주기
	 */
