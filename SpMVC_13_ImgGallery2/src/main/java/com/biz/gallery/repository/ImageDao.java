...
public interface ImageDao {
	...
	/*
	 * @SelectKey(keyProperty = "img_seq", keyColumn = "img_seq", resultType =
	 * Long.class, statement =
	 * "SELECT * FROM tbl_gallery WHERE img_seq = #{img_seq}", before = false)
	 */

	// before가 true이면 insert가 수행되기 전, false이면 수행된 다음

	@InsertProvider(type = ImageSQL.class, method = "insert_sql")
	public int insert(ImageVO imageVO);

	@Select("SELECT * FROM tbl_gallery WHERE img_seq = #{data_seq}")
	@Results(value = { @Result(property = "img_seq", column = "img_seq"),
			@Result(property = "img_files", column = "img_seq", javaType = List.class, many = @Many(select = "getFiles")) })
	public ImageVO findBySeq(String data_seq);
	...
	public int update(ImageVO imgVO);
